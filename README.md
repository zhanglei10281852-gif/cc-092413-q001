...
震后联络档案中的居民可能关联一笔或多笔救助事务，删除流程区分两种操作，均要求请求头携带操作者标识 `X-Operator`（ASCII 工号或用户名）：

- `DELETE /residents/{id}`：仅当居民没有关联事务时允许删除。存在关联事务时返回 `409`，错误体使用稳定错误码 `resident_has_affairs`，`context` 中给出档案标识、阻止删除的事务清单以及安全路径提示；拒绝尝试本身也会写入审计（`outcome=denied`）。
- `POST /residents/{id}/archive`：显式归档清理的安全路径，请求体必须包含 `"confirm": true`。操作在单个即时事务内将居民与关联事务快照写入 `resident_archives` / `affair_archives`，再删除活动数据；任何一步失败都会整体回滚，不会留下半删除状态。成功操作写入 `resident.archive` 审计，包含操作者、居民档案标识、归档编号与归档事务数。
